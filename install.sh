...
configure_services() {
  ...
  log "Disabling SwayNC so AGS handles notifications"

  systemctl --user mask --now swaync.service 2>/dev/null || true
...
link_configuration() {
  ...
  link_path \
    "$repo/quickshell/section9" \
    "$config/quickshell/section9"
}
...
configure_neovim() {
  log "Bootstrapping Neovim plugins and Tree-sitter parsers"

  if ! command -v nvim >/dev/null 2>&1; then
    warn "Neovim is not installed; skipping Neovim bootstrap"
    return 0
  fi

  if [[ ! -e "$config/nvim/init.lua" ]]; then
    warn "Neovim init.lua is unavailable; skipping bootstrap"
    return 0
  fi

  local parser_dir="$HOME/.local/share/nvim/site/parser"
  local cleanup_marker="$HOME/.local/state/section9/nvim-parser-cleanup-v1"

  mkdir -p "$(dirname "$cleanup_marker")"

  # Only move old parsers once.
  if [[ -d "$parser_dir" && ! -e "$cleanup_marker" ]]; then
    local parser_backup
    parser_backup="$HOME/.local/share/nvim/site/parser.backup-$(date +%Y%m%d-%H%M%S)"

    log "Moving existing Tree-sitter parsers to $parser_backup"
    mv "$parser_dir" "$parser_backup"
  ...
  if ! nvim --headless "+Lazy! sync" +qa; then
    warn "Lazy plugin synchronization returned an error"
    warn "Run Neovim manually and inspect :Lazy"
    return 0
  fi
  ...
  if nvim --headless "+TSUpdateSync" +qa; then
    printf 'Tree-sitter parsers updated successfully.\n'
  else
    warn "TSUpdateSync failed or is unavailable"
    warn "Inspect :checkhealth vim.treesitter inside Neovim"
  fi
}
